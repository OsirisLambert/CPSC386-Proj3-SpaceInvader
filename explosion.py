...
class Ship_explosion(Sprite):
    def __init__(self, screen, ship, image_path1, image_path2, image_path3):
        super(Ship_explosion, self).__init__()
        self.images = []
        self.create_animate_image(ship, image_path1, image_path2, image_path3)
        self.image_timer = Timer(self.images, 100, 0, 2, True)
        self.image = self.image_timer.imagerect()
        self.rect = self.image.get_rect(topleft=(ship.rect.x - 15, ship.rect.y - 20))

    def create_animate_image(self, ship, image_path1, image_path2, image_path3):
        self.temp1 = pygame.image.load(image_path1)
